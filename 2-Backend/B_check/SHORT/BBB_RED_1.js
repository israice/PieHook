...
import { check_closed_SHORT_RED_1 } from './BBBA_check_closed_SHORT_RED_1.js';
...
async function short_List() {
  await check_closed_SHORT_RED_1();
}

async function SHORT_RED_1() {
  try {
    const content = await fs.readFile(SETTINGS_FILE, 'utf8');
    const config = yaml.parse(content);

    if (!(SHORT_VARIABLE in config)) {
      console.error(`❌ Ошибка: переменная ${SHORT_VARIABLE} не найдена в ${SETTINGS_FILE}`);
      return;
    }

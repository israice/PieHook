// core/B_check/Binance/GET_orders/sell_long_market_body.js
...
const CONFIG = {
  ...
  ORDERS_BODY_KEY: 'sell_long_market_body'
};

dotenv.config({ path: CONFIG.DOTENV_FILE });

// ============================================================
// ОСНОВНАЯ ФУНКЦИЯ — ОТПРАВКА МАРКЕТ-ОРДЕРА LONG SELL
// ============================================================
async function sell_long_market_body() {
  try {
    ...
    if (!baseOrder) {
      console.error('❌ Не найдено тело ордера sell_long_market_body в orders_collection.yaml');
      return;
    }
    ...
    if (!selectedPath.includes('Binance\\')) {
      console.error(`❌ Не удалось извлечь имя аккаунта из пути: ${selectedPath}`);
      return;
    }
    const account = selectedPath.split('Binance\\')[1].split('\\')[0];
    ...
    // ← МИНИМАЛЬНОЕ ИЗМЕНЕНИЕ: жёстко ставим precision = 0, чтобы избежать ошибки Precision over…
    const allowedPrecision = 0;
    ...
    // 7. Вычисление подписи и отправка запроса на Binance API
    ...
    // Ошибки умалчиваются, как в оригинале
  }
}

// === ЭКСПОРТ ФУНКЦИИ ===
export { sell_long_market_body };

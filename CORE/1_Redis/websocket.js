...
import { mergeMap, tap, takeUntil, delay, retry } from "rxjs/operators";

// --- Constants ---
const SETTINGS_FILE = "/app/CORE/1_Redis/websocket_config.yaml";
...
async function createRedisClient(settings) {
  ...
  console.log(
    `Connected to Redis at ${settings.redis_host}:${settings.redis_port}`
  );
  return client;
}
...
async function handleMessages(msg, redisClient, redisKey) {
  try {
    ...
    console.error(
      "Error processing message (not valid JSON or other error):",
      err.message
    );
  }
}
...
async function binanceWebSocketToRedis(settings, redisClient) {
  ...
  const connectWebSocket = () => {
    ...
    const close$ = fromEvent(ws, "close").pipe(
      tap(([code, reason]) => {
        console.warn(
          `WebSocket closed (${code}): ${reason}. Reconnecting in ${
            reconnectDelay / 1000
          }s`
        );
      })
    ...
    merge(open$, message$, close$, error$, reconnectStream$)
      .pipe(
        ...
        retry({
          delay: (err) => {
            console.error("WebSocket stream error, retrying...", err.message);

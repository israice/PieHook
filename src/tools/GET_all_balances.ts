...
async function fetchPosition(account_type: string, symbol: string): Promise<{
  account_type: string;
  symbol: string;
  hedge_mode_enabled: boolean;
  positions: Position[];
  ...
  const allPositions = await res.json() as Position[];
...
async function fetchBalance(account_type: string): Promise<number> {
  const apiKey = process.env[`${account_type}_API_KEY`];
  const apiSecret = process.env[`${account_type}_API_SECRET`];
  if (!apiKey || !apiSecret) {
    throw new Error(`❌ Нет API_KEY или API_SECRET для ${account_type} в .env`);
  ...
  const balances = await res.json() as { asset: string; balance: string }[];
...
async function GET_all_balances(): Promise<void> {
  ...
  const enriched: EnrichedPosition[] = results.map(r => ({
